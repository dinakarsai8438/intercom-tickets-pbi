...
adls_path =f"abfss://intercom-conversation-history@nourishinternal.dfs.core.windows.net/incremental_raw_json/"
...
def sanitize_nested_schema(schema: StructType) -> StructType:
    sanitized_fields = []
    for field in schema.fields:
        # Remove problematic characters
        sanitized_name = re.sub(r'[ :?(),/.!]', '', field.name)
        ...
        if isinstance(field.dataType, StructType):
            sanitized_fields.append(StructField(sanitized_name, sanitize_nested_schema(field.dataType), field.nullable))
        
        # Handle nested arrays
        elif isinstance(field.dataType, ArrayType) and isinstance(field.dataType.elementType, StructType):
            sanitized_fields.append(StructField(sanitized_name, ArrayType(sanitize_nested_schema(field.dataType.elementType)), field.nullable))
        
        # Handle map types (keys and values)
        elif isinstance(field.dataType, MapType):
            sanitized_fields.append(StructField(sanitized_name, MapType(field.dataType.keyType, sanitize_nested_schema(field.dataType.valueType)), field.nullable))
        
        # Handle simple types
        else:
            sanitized_fields.append(StructField(sanitized_name, field.dataType, field.nullable))

    return StructType(sanitized_fields)


# In[208]:


sanitized_schema = sanitize_nested_schema(df.schema)
df = spark.createDataFrame(df.rdd, sanitized_schema)
...
#json_files = [file.path for file in file_paths if file.isFile]  # Ensure we only process files

# Initialize a list to log problematic files
#problematic_files = []
 ...
 #   try:
  ...
  #      df = spark.read.option("multiline", "true").json(file_path)
   ...
   #     df_flattened = flatten(df)
    #except Exception:
        # Log only the file path of problematic files
...
# Print only the problematic file names 
...
output_path = (
    f"abfss://intercom-conversation-history@nourishinternal.dfs.core.windows.net/"
    f"incremental_conversation_history_parquet/conversation_history_rest_of_fields/"
)
...
input_path_1 = (
    "abfss://intercom-conversation-history@nourishinternal.dfs.core.windows.net/"
    "schema/conversation_history_schema/conversation_history_rest_of_fields/*.json"
)
...
if difference_list_1:
    ...
    add_column_ddl_query_1 = f"ALTER TABLE conversation_history_rest_of_fields ADD {', '.join(new_columns_1)};"
    ...
    output_path_1 = (
        f"abfss://intercom-conversation-history@nourishinternal.dfs.core.windows.net/"
        f"schema/conversation_history_schema/conversation_history_rest_of_fields/"
    )
...
for array_field in array_parents:
    ...
    output_path_array = (
        f"abfss://intercom-conversation-history@nourishinternal.dfs.core.windows.net/"
        f"incremental_conversation_history_parquet/conversation_history_{array_field}/"
    )
    ...
    input_path_array = (
        f"abfss://intercom-conversation-history@nourishinternal.dfs.core.windows.net/"
        f"schema/conversation_history_schema/conversation_history_{array_field}/*.json"
    )
    ...
    if difference_list:
        ...
        # Generate ALTER TABLE statement without parentheses
        add_column_ddl_query = f"ALTER TABLE conversation_history_{array_field} ADD {', '.join(new_columns)};"
        ...
        output_path_schema = (
            f"abfss://intercom-conversation-history@nourishinternal.dfs.core.windows.net/"
            f"schema/conversation_history_schema/conversation_history_{array_field}/"
        )
...
#print(add_column_ddl)


# In[234]:


dummy_query=["SELECT TOP(1) * FROM [dbo].[conversation_history_contacts];"]
...
if add_column_ddl:
    ...
    output_path = f"abfss://intercom-conversation-history@nourishinternal.dfs.core.windows.net/incremental_ddl/conversation_history_ddl"
